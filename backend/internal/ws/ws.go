...
const (
	// sendBuffer is how many positions may be queued for a client before it is
	// considered too far behind. Positions are superseded by the next one, so a
	// backlog this deep is already stale.
	sendBuffer = 256
...
type client struct {
	conn *websocket.Conn
	send chan gpspoints.CreateGpsPoint
...
func (h *Hub) add(conn *websocket.Conn) *client {
	c := &client{
		conn: conn,
		send: make(chan gpspoints.CreateGpsPoint, sendBuffer),
...
// Broadcast hands the position to every client's queue and returns. It performs
// no I/O, so one slow client cannot delay any other.
func (h *Hub) Broadcast(m gpspoints.CreateGpsPoint) {
	...
	for _, c := range clients {
		...
		default:
			// queue full: drop this position rather than stall the broadcast.
